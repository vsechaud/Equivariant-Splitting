# Adapted from https://github.com/deepinv/deepinv/blob/2c61a518af42069535671b3c5a89694e7ef86c54/deepinv/training/trainer.py
    ...
    **Model Saving**, **Comparing with Pseudoinverse Baseline**, **Plotting**, **Verbose** and **Weights & Biases**.
    ...
    def log_metrics_wandb(self, logs: dict, step: int, train: bool = True):
        ...
        if train and self.optimizer is not None:
            ...
            if len(param_groups) == 1:
                logs["Learning rate"] = param_groups[0]["lr"]
            else:
                warnings.warn(
                    "Multiple parameter groups detected. Not logging learning rate to mlops tools."
                )
        ...
        if save_images:
            ...
            for k, img in enumerate(imgs):
                for i in range(img.size(0)):
                    ...
                    save_image(
                        img[i].unsqueeze(0), img_name + f"{self.img_counter + i}.png"
                    )
    ...
    def load_best_model(self):
        ...
        if not self.save_path:
            ...
            ckp_path = Path(self.save_path) / Path("ckp_best.pth.tar")
            ckp_path = str(ckp_path)
            self.load_model(ckpt_pretrained=ckp_path)
            if self.mlflow_vis:
                mlflow.log_param("best_checkpoint_path", ckp_path)
    ...
    def test(
        ...
        for i in (
            ...
            self.step(0, progress_bar, train=False, last_batch=(i == 0))
